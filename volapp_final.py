...
def create_features(window_data):
    ...
    window_data['noise_ratio'] = window_data['ask_price1'].diff().abs() / window_data['WAP'].diff().abs().rolling(5).std()
...
#@st.cache_data(ttl=3600)
def simulate_real_time_forecasting(full_data, lookback_seconds=200, max_horizon=30, _progress_bar=None, _status_text=None):
    all_predictions = []
    ...
    total_steps = len(range(lookback_seconds, len(full_data) - max_horizon))
    for step, current_pos in enumerate(range(lookback_seconds, len(full_data) - max_horizon)):
        # Update progress bar and status text
        if _progress_bar:
            progress = int((step / total_steps) * 100)
            _progress_bar.progress(progress)
            if _status_text:
                _status_text.text(f"Processing: {progress}% complete")

        # Simulate feature creation and model training
        lookback_window = full_data.iloc[current_pos - lookback_seconds:current_pos]
        future_window = full_data.iloc[current_pos:current_pos + max_horizon]
        ...
        if current_pos == lookback_seconds:
    ...
    if _progress_bar:
        ...
        if _status_text:
            _status_text.text("Processing complete!")
...
if selected_id:
    ...
    tab1, tab2, tab3 = st.tabs(["📈 Dashboard", "🤖 Prediction Model", "Risk Calculator"])
    ...
    with tab2:
        ...
        # Simulate real-time forecasting with progress updates
        full_results = simulate_real_time_forecasting(stock_data, n_past, _progress_bar=progress_bar, _status_text=status_text)
        ...
        time_diffs = np.abs(full_results['timestamp'] - pd.to_datetime(selected))
        closest_idx = time_diffs.idxmin()
        selected_timestamp = full_results.loc[closest_idx, 'timestamp']
        
        lookback_start = full_results.iloc[closest_idx]['timestamp'] - pd.Timedelta(seconds=n_past)
        forecast_end = full_results.iloc[closest_idx]['timestamp'] + pd.Timedelta(seconds=n_focus)

        
        # Get indices with boundary checks
        lookback_start_idx = max(0, full_results['timestamp'].searchsorted(lookback_start))
        forecast_end_idx = min(len(full_results), full_results['timestamp'].searchsorted(forecast_end))

        lookback_data = full_results.iloc[lookback_start_idx:closest_idx]
        forecast_data = full_results.iloc[closest_idx:forecast_end_idx]
        ...
        if not full_results.empty:
            # Convert the predictions DataFrame to CSV
            ...
            st.download_button(
                ...
                mime="text/csv",
                help="Download the predictions as a CSV file for further analysis."
            )
    ...
    with tab3:
        st.subheader("Advanced Volatility Trading Calculator")
        st.markdown("""
        <style>
        .big-font { font-size:18px !important; }
        .highlight-box { padding: 15px; border-radius: 10px; background-color: #f0f2f6; }
        </style>
        """, unsafe_allow_html=True)
        ...
        with col1:
            ...
            capital = st.number_input("💰 Total Capital ($)", value=100000, step=10000, 
                                   help="The total amount you're allocating for trading.")
            risk_per_trade = st.slider("⚖️ Max Risk per Trade (%)", 0.1, 5.0, 1.0, 0.1,
                                     help="Percentage of capital you're willing to risk per trade.")
            stop_loss_pct = st.slider("🛑 Stop Loss Distance (%)", 0.1, 10.0, 2.0, 0.1,
                                    help="Distance from entry price for stop loss placement.")
            
            # Get the latest volatility prediction from tab2 if available
            if 'full_results' in locals() and not full_results.empty:
                last_pred_time = forecast_data['timestamp'].iloc[-1]
                
                st.markdown(f"""
                <div class="time-context">
                    <strong>⏱ Prediction Context:</strong> This calculator uses the <strong>final predicted volatility</strong> at {last_pred_time.strftime('%H:%M:%S')} based on a {n_focus}-second forecast horizon.<br>
                </div>
                """, unsafe_allow_html=True)
                latest_volatility = forecast_data.iloc[-1]['predicted_volatility'] * 100  # Convert to percentage
                forecast_vol = st.number_input(
                    "📉 Forecasted Volatility (%)",
                    value=float(latest_volatility),
                    format="%.4f",
                    help="Model-predicted short-term volatility (auto-populated from prediction tab)"
                )
            else:
                st.markdown("""
                <div class="time-context" style="background-color:#fff2f0;border-left-color:#ff4d4f;">
                    <strong>⚠️ No Live Data:</strong> Using default values until volatility model runs in Tab 3
                </div>
                """, unsafe_allow_html=True)
                forecast_vol = st.number_input(
                    "📉 Forecasted Volatility (%)",
                    value=1.5,
                    format="%.4f",
                    help="Model-predicted short-term volatility"
                )
        
        with col2:
            st.markdown("### 🎯 Strategy Parameters")
            leverage = st.slider("🏦 Leverage", 1, 10, 1, 
                               help="Amount of leverage to apply (1x = no leverage)")
            asset_price = st.number_input("💵 Current Asset Price ($)", value=100.0, step=0.1,
                                        help="Current price of the asset you're trading")
            trading_fee = st.number_input("💸 Trading Fee (%)", value=0.1, step=0.01,
                                        help="Percentage fee per trade (both entry and exit)")
        
        # Calculations
        risk_dollar = capital * (risk_per_trade / 100)
        stop_loss_distance = asset_price * (stop_loss_pct / 100)
        position_size = (risk_dollar / stop_loss_distance) * leverage
        position_value = position_size * asset_price
        capital_usage = (position_value / capital) * 100
        fee_cost = position_value * (trading_fee / 100) * 2  # Entry and exit
        
        # Risk assessment
        risk_ratio = (forecast_vol / stop_loss_pct) * 100
        if risk_ratio > 150:
            risk_assessment = "🔥 Extremely Risky (Volatility much larger than stop)"
        elif risk_ratio > 120:
            risk_assessment = "⚠️ High Risk (Volatility larger than stop)"
        elif risk_ratio > 80:
            risk_assessment = "🟡 Moderate Risk"
        else:
            risk_assessment = "🟢 Low Risk (Stop larger than volatility)"
        
        # Results display
        st.markdown("### 📊 Trading Plan Analysis")
        ...
        with results_col1:
            ...
            <div class="highlight-box">
                <p class="big-font"><strong>Position Sizing:</strong></p>
                <p>Max risk per trade: <strong>${risk_dollar:,.2f}</strong></p>
                <p>Position size: <strong>{position_size:,.2f} units</strong></p>
                <p>Position value: <strong>${position_value:,.2f}</strong></p>
                <p>Capital usage: <strong>{capital_usage:.1f}%</strong></p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with results_col2:
            st.markdown(f"""
            <div class="highlight-box">
                <p class="big-font"><strong>Risk Analysis:</strong></p>
                <p>Stop distance: <strong>{stop_loss_pct:.2f}% (${stop_loss_distance:.2f})</strong></p>
                <p>Volatility/Stop ratio: <strong>{risk_ratio:.1f}%</strong></p>
                <p>Risk assessment: <strong>{risk_assessment}</strong></p>
                <p>Estimated fees: <strong>${fee_cost:,.2f}</strong></p>
            </div>
            """, unsafe_allow_html=True)
        ...
        daily_move = asset_price * (forecast_vol / 100)
        expected_1h_move = daily_move * (1/24)**0.5  # Assuming 24h market
        ...
        with col_adv2:
            st.metric("Expected 1h Move", f"±${expected_1h_move:.2f}")
        with col_adv3:
            profit_target = stop_loss_pct * 1.5  # 1.5:1 reward:risk
            st.metric("Suggested Profit Target", f"{profit_target:.1f}% (${asset_price * (profit_target/100):.2f})")
        
        # Connection to volatility predictions
        st.markdown("### 🔗 Volatility Prediction Insights")
        if 'full_results' in locals() and not full_results.empty:
            vol_trend = "↑ Increasing" if forecast_data['predicted_volatility'].iloc[-1] > forecast_data['predicted_volatility'].iloc[-2] else "↓ Decreasing"
            st.markdown(f"""
            - Current volatility trend: <strong>{vol_trend}</strong>
            - Recent volatility range: {forecast_data['predicted_volatility'].min()*100:.2f}% to {forecast_data['predicted_volatility'].max()*100:.2f}%r
            """, unsafe_allow_html=True)
            
            if st.button("🔄 Refresh with Latest Prediction"):
                # This would trigger a recalculation with the newest data
                st.experimental_rerun()
        else:
            st.info("Run the volatility prediction model in Tab 3 to get enhanced insights")
        
        # Disclaimer
        st.markdown("""
        <div style="font-size:12px; color:#666; margin-top:20px;">
        <strong>Note:</strong> This calculator provides estimates only. Actual market conditions may vary. 
